...
app.post('/register', (req, res) => {
  const { username, phone_number, password, first_name, last_name, age } =
    req.body;

  const query =
    'INSERT INTO users (username, phone_number, password, first_name, last_name, age) VALUES (?, ?, ?, ?, ?, ?)';
  connection.query(
    query,
    [username, phone_number, password, first_name, last_name, age],
    (err, result) => {
      if (err) {
...
app.post('/candidate-delete', (req, res) => {
  ...
  connection.query(findVotersQuery, [id], (err, voters) => {
    ...
    if (userIds.length > 0) {
      ...
      const resetVotersQuery =
        'UPDATE users SET has_voted = FALSE WHERE id IN (?)';
...
app.post('/delete-all-candidates', (req, res) => {
  ...
  connection.query(resetVotersQuery, (err, result) => {
    ...
    connection.query(deleteCandidatesQuery, (err, result) => {
      ...
      res.json({
        success: true,
        message: 'All candidates deleted and voter status reset successfully',
      });
    });
  });
});
